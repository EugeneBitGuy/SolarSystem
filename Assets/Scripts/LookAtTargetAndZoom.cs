...
public class LookAtTargetAndZoom: MonoBehaviour {
    ...
    [Tooltip("This is the speed at which you'll zoom in/out to the object")]
    public float scrollSpeed;

    [Tooltip("Minimap camera where you can click on object to look at it")]
    public Camera minimapCamera;

    [Tooltip("Text Panel where is shown the object name")]
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (minimapCamera != null)
                hits = hits.Concat(Physics.RaycastAll(minimapCamera.ScreenPointToRay(Input.mousePosition))).OrderBy(hit => hit.distance)
                    .ToArray();
       ...
       // if a currentTarget is set, then look at it
        if (currentTarget!=null)
        {
            ...
            textPanel.text = currentTarget.name;
        ...
        var scrollVelocity = Input.GetAxis("Mouse ScrollWheel");
        if (scrollVelocity != 0)
        {
            Camera.main.fieldOfView -= scrollVelocity * scrollSpeed;
            Camera.main.fieldOfView = Camera.main.fieldOfView > 120
                ? 120
                : (Camera.main.fieldOfView < 1 ? 1 : Camera.main.fieldOfView);
